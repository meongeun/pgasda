...
import torchvision
from models.layers import disp_to_depth, transformation_from_parameters
...
def init_net(net, init_type='normal', gpu_ids=[]):
    # Data parallel code here 
    if len(gpu_ids) > 0:
        assert(torch.cuda.is_available())

        print("Utilizing gpu : {}".format(gpu_ids))
        net.to(gpu_ids[0])
...
class ReconImgLoss(nn.Module):
    def __init__(self, alpha=0.85):
        super(ReconImgLoss, self).__init__()
        self.alpha = alpha

    def forward(self, backproject_depth, project_3d, img, outputs, K, inv_K, device):
        wimg = {}
        total_loss = 0.0
        oimg = img[("color", 0, -1)]
        _,_, h, w = oimg.shape
        ...
        # disp = outputs[('depth')].cpu().detach().float().numpy()
        #
        # print(disp_gt.max(), disp_gt.min())
        ...
        # disp= outputs[('depth')]
        # .cpu().detach().float().numpy()
        _, depth = disp_to_depth(disp, 0.01, 100)
        ...
        for i, frame_id in enumerate([1,-1]):
            ...
            # exit()
            # inv_depth = 1 / depth
            # mean_inv_depth = inv_depth.mean(3, keepdims=True).mean(2, keepdims=True)
            ...
            #print(depth.dtype, inv_K.dtype)
            cam_points = backproject_depth(depth, inv_K)
            #print(K.dtype, cam_points.dtype)
            #exit()
            pix_coords = project_3d(cam_points, K, T)

            wimg[("color", frame_id, -1)] = F.grid_sample(img[("color", frame_id, -1)], pix_coords, padding_mode="border")

            ssim_ = ssim(oimg, wimg[("color", frame_id, -1)])
            l1 = torch.abs(oimg-wimg[("color", frame_id, -1)])
            loss = torch.mean(self.alpha * ssim_)+torch.mean((1-self.alpha) * l1)

            total_loss += loss

        return total_loss, wimg
