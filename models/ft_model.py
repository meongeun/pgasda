...
class FTModel(BaseModel):
    ...
    def forward(self):
        ...
        if self.isTrain:
            ...
            self.out = self.netG_Depth_T(torch.cat((self.src_img[('color', 0, -1)], self.fake_src_img[('color', 0, -1)]), 0))
    ...
    def backward_G(self):
        ...
        for f_i in self.opt.frame_idxs[1:]:
            ...
            self.loss_R_Pose_Src += lambda_R_Pose * (self.criterionPoseReg(src_q,self.src_gt[("axisangle", 0, f_i)])
                                                  + self.criterionPoseReg(src_t, self.src_gt[("translation", 0, f_i)]))
