...
def transformation_from_depth_parameters(depth, axisangle, translation, frame_id):
    inv_depth = 1 / depth
...
class BackprojectDepth(nn.Module):
    ...
    def forward(self, depth, inv_K):
        ...
        #print(depth.shape, self.pix_coords.shape, inv_K[:,:3,:3].shape, cam_points.shape)
        #print(depth.view(self.batch_size, 1, -1).shape, cam_points.shape)
...
class Project3D(nn.Module):
    ...
    def forward(self, points, K, T):
        #print(K.dtype, T.dtype)
        #K = K.double()
        T = T.float()
        P = torch.matmul(K, T)[:, :3, :].float()
        #print(P.dtype, points.dtype)
...
def compute_depth_errors(gt, pred):
    ...
    return abs_rel, sq_rel, rmse, rmse_log, a1, a2, a3, l1
